...
FMI_variants_PTEN_MTL <-
  ...
  mutate(
    tot_effect = ifelse(
      description_v2 %in% filter(PTEN_data_LOF_MTL, tot_effect == "lof")$description_v2,
      "lof",
      NA
    ),
    tot_effect = ifelse(
      description_v2 %in% filter(PTEN_data_LOF_MTL, tot_effect == "wt")$description_v2,
      "wt",
      tot_effect
    ),
    tot_effect = ifelse(codingType_SV == "splice", "lof", tot_effect)
  ) %>%
  left_join(PTEN_hotspots_MTL) %>%
  mutate(htsp_status = replace_na(htsp_status, "non-htsp")) %>%
  select(-codon_SV, -codingType_SV)


table(filter(FMI_variants_PTEN_MTL, tot_effect == "lof")$htsp_status)
# htsp non-htsp 
# 574      738
table(filter(FMI_variants_PTEN_MTL, tot_effect == "wt")$htsp_status)
# htsp non-htsp 
# 10       74
table(filter(FMI_variants_PTEN_MTL, is.na(tot_effect))$htsp_status)
# htsp non-htsp 
# 13       31
