...
# Predicted data ---------------------------------------------------------------
MAVE_score_pred_MTL <- c()
for (i in 1:nrow(pred_MTL_prop)) {
  MAVE_score_pred_MTL <- c(MAVE_score_pred_MTL,
                           rep(pred_MTL_prop$MAVE_score[i],
                               pred_MTL_prop$prop_pred[i]))
}

pred_MTL_prop_MAVE <- tibble(MAVE_score = MAVE_score_pred_MTL,
                             data = "pred")
...
Panel_B_data <- bind_rows(pred_MTL_prop_MAVE,
                          select(PTEN_lof_data_MTL, MAVE_score, data)) %>%
  mutate(data = as.factor(data))
# Draw the chart Panel B -------------------------------------------------------
Panel_B_data %>%
  ggplot(aes(x = MAVE_score, fill = data)) +
...
group_by(PTEN_lof_data_MTL, alt) %>%
  ...
  bind_rows(mutate(rename(pred_MTL_prop, count = prop_pred),
                   data = "predicted")) %>%
  write.csv("../Charts, data, statistics/Panel B observed and predicted data.csv",
            row.names = F)

Panel_B_data %>%
  write.csv("../Charts, data, statistics/Panel B chart data.csv",
            row.names = F)
