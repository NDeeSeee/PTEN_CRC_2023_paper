...
TCGA_mutations_msi = fread("../KRAS-project/PAD_preprocessing/Oncopanels/TCGA_msi_prediction_completed_expanded.csv")
...
#Validation case
val_panel_name = "MSK-IMPACT341"

train_data = get_panel_mut_count(val_panel_name)

#train_data %>%
#  write_csv("~/panel_341_train_data.csv")

# Here we decided to pick 0.005 p-value as threshold
model_341_R = ctree(msi_final ~ ., train_data, controls = ctree_control(mincriterion = 0.995))
plot(model_341_R)
#ggsave("MSK-IMPACT341 tree model.png")

# remove external duplicates from PAD_2022_rebuild.R
MSC_samples = filter(MSC_samples, SAMPLE_ID %in% MSC_pat_full$sample_id)

MSC_samples_341 = filter(MSC_samples, GENE_PANEL == "IMPACT341") %>%
...
MSK_mutations_341 = filter(MSC_mutations, Consequence != "synonymous_variant") %>%
  ...
  left_join(y = select(MSC_samples_341, sample_id, msi_mskcc, msi_score), by ="sample_id") %>%
...
MSK_test_341 = group_by(MSK_mutations_341, sample_id) %>%
...
MSK_test_341_origin = MSK_test_341 %>%
  select(-msi_final, -mut_count, -msi_score) %>%
  rename(msi_final = msi_mskcc, mut_count = mut_count_native)

#MSK_test_341_origin %>%
#  write_csv("~/panel_341_test_data.csv")

#Predict with R and metrics
true_values = filter(MSK_test_341_origin, !is.na(msi_final))$msi_final
predicted_values_probs = predict(model_341_R, select(filter(MSK_test_341_origin, !is.na(msi_final)), -msi_final), type = "prob")
